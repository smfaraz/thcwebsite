...
export const RegionalExpertise: React.FC = () => {
  const regions = [
    ...
    { name: "United Arab Emirates", code: "UAE", desc: "Serving the growing Oil and Gas sector in the UAE with top-tier talent solutions." },
          ...
          <div className="lg:w-1/2">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-white/10 p-4 rounded-lg backdrop-blur-sm border border-white/10">
                <h4 className="font-bold text-2xl text-thc-orange">4+</h4>
                <p className="text-sm text-gray-300">Key Markets (KSA, USA, UAE, IND)</p>
              </div>
              <div className="bg-white/10 p-4 rounded-lg backdrop-blur-sm border border-white/10">
          ...
          <div className="lg:w-1/2 grid grid-cols-1 md:grid-cols-2 gap-6">
            {regions.map((region, idx) => (
              <FadeIn key={idx} delay={idx * 150} className="h-full">
                <div className="h-full bg-white/5 p-6 rounded-lg border border-white/10 hover:bg-white/10 transition-colors">
